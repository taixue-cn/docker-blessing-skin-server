...
For a zero-click migration, the Taixue `profile` scope should emit an integer `bs_uid` claim from the accepted `BS` identity binding. Because the claim is inside the signed ID Token, the plugin may use it to create the local `sub` mapping. A conflicting `bs_uid`, local UID, or `sub` is rejected; email and nickname remain display-only attributes.
